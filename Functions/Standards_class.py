...
class SampleAnalysis:
    def __init__(self, sheet_name, table_range, wb=None):
        ...
        Parameters:
        wb (xw.Book or None): The Excel workbook (optional for testing).
        """
    ...
    def extract_data(self):
        """Extracts sample data from the specified range."""
        sample_names = []
        replicates = []
        sample_types = []
        dilutions = []
    ...
    def generate_well_labels(self):
        """Generates well labels based on the DataFrame row count."""
    ...
    def organize_dataframe(self):
        """Rearranges the columns and organizes the final DataFrame."""
    ...
    def process_data(self):
        """Processes data without Ct values."""
        self.open_excel()
        self.extract_data()
    ...
    def process_data_ct(self, ct_file_path, ct_sheet_name, ct_column_name='Cq'):
        """Processes data and includes Ct values."""
...
def calculate_dilution_summary(df):
    ...
    # Format 'Dilution' to scientific notation with two decimal places
    dilution_summary['Dilution'] = dilution_summary['Dilution'].map(lambda x: "{:.2E}".format(x))

    # Calculate log of Dilution, round to two decimal places, and add as a new column
